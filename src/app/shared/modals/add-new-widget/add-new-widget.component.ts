...
export class AddNewWidgetComponent implements OnInit {
  data: { projectView: ProjectView; widget?: Widget } = inject(MAT_DIALOG_DATA);
  ...
  dialogRef: MatDialogRef<AddNewWidgetComponent> = inject(MatDialogRef<AddNewWidgetComponent>);
  ...
  installOrDeployForm: FormGroup<InstallOrDeployWasmForm> = new FormGroup<InstallOrDeployWasmForm>({
    ...
    fileData: new FormControl<string | null>(null, [Validators.required]),
  });
      ...
      switch (this.data.widget.type) {
        ...
        case WidgetType.DEPLOY_CONTRACT:
          this.installOrDeployForm.patchValue({
            fileData: this.data.widget.fileData,
  ...
  confirm(): void {
    ...
    switch (this.baseForm.value.type as WidgetType) {
      ...
      case WidgetType.INSTALL_WASM:
        ...
        newWidget = {
          _id: widgetId,
          project: this.baseForm.value.project!,
          projectView: this.baseForm.value.projectView!,
          name: this.baseForm.value.name!,
          type: WidgetType.INSTALL_WASM,
          fileData: this.installOrDeployForm.value.fileData!,
          fileName: this.installOrDeployForm.value.fileName!,
          source: this.installOrDeployForm.value.source!,
  ...
  async onFileSelected(e: Event): Promise<void> {
    ...
    this.installOrDeployForm.controls.fileName.setValue(file.name, { emitEvent: false });
    const base64 = Buffer.from(await file.arrayBuffer()).toString('base64');
    this.installOrDeployForm.controls.fileData.setValue(base64, { emitEvent: false });
  }
}
